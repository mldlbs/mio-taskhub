...
assert(r.total === 210, `total=${r.total} expected 210`)
assert(r.criticalPath.join(',') === 'a,b,d', `path=${r.criticalPath}`)
assert(r.critical.a && r.critical.b && r.critical.d && !r.critical.c, 'critical flags')
assert(r.float.c === 30, `float.c=${r.float.c}`)
...
assert(r2.total === 60, `done total=${r2.total}`)

// 无 est_duration 用默认值
const noEst = [{ id: 'n', depends_on: [], state: 'queued', stage: 'ready' }]
const r3 = computeCPM(noEst)
assert(r3.total === 60, `noEst total=${r3.total}`)
...
assert(r4.total === 30, `dangling total=${r4.total}`)
...
assert(JSON.stringify(paths5) === JSON.stringify(['a,b,d', 'c,d']),
  `allCriticalPaths=${JSON.stringify(paths5)} expected ['a,b,d','c,d']`)
assert(r5.critical.a && r5.critical.b && r5.critical.c && r5.critical.d,
...
assert(JSON.stringify(seg6) === JSON.stringify(['0-120:2', '120-180:1', '180-210:1']),
  `parallelProfile=${JSON.stringify(seg6)}`)
assert(r6.parallel === 2, `parallel=${r6.parallel}`)

// resourceConflicts: 无 target_agent_type 时默认无阈值 → 空
assert(r6.resourceConflicts.length === 0, `默认无 agent 信息 resourceConflicts=${r6.resourceConflicts.length}`)
...
assert(JSON.stringify(seg7) === JSON.stringify(['0-120:2']), `resourceConflicts=${JSON.stringify(seg7)}`)
...
assert(r8.resourceConflicts.length === 0, `默认 agent 阈值 resourceConflicts=${r8.resourceConflicts.length}`)

// 环防护：不崩溃，返回空关键路径
const cyc = computeCPM([
  { id: 'x', est_duration_min: 0,  depends_on: [], state: 'queued', stage: 'ready' },
  { id: 'a', est_duration_min: 30, depends_on: ['x', 'b'], state: 'queued', stage: 'ready' },
  { id: 'b', est_duration_min: 30, depends_on: ['a'], state: 'queued', stage: 'ready' },
])
assert(Array.isArray(cyc.allCriticalPaths), `环输入 allCriticalPaths 应为数组, got ${cyc.allCriticalPaths}`)

// 空任务
const empty = computeCPM([])
assert(empty.total === 0 && empty.parallelProfile.length === 0 &&
  empty.resourceConflicts.length === 0 && empty.allCriticalPaths.length === 0, '空任务输入')
